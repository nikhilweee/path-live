...
enum Setting<T> {
  // App configuration
  authKey<String>('auth_key', "603b1ed9-b5ab-4827-8606-6b501cf50d9e", StorageType.string),
  dbChecksum<String>('db_checksum', "499C6586DE8D66E0028A71F21EB9E55C", StorageType.string),
  ...
  try {
    ...
    switch (setting.storageType) {
      ...
      case StorageType.stringList:
        return (preferences.getStringList(key) ?? defaultValue) as T;
      
      case StorageType.dateTime:
        final timestamp = preferences.getInt(key);
        return (timestamp != null ? DateTime.fromMillisecondsSinceEpoch(timestamp) : defaultValue) as T;
    }
  } catch (e) {
...
Future<void> resetAllTrainSettings() async {
  await Future.wait([
    setSetting(Setting.previousTrainsCount, Setting.previousTrainsCount.defaultValue),
    setSetting(Setting.futureTrainsCount, Setting.futureTrainsCount.defaultValue),
    setSetting(Setting.progressBarDuration, Setting.progressBarDuration.defaultValue),
    setSetting(Setting.countdownThreshold, Setting.countdownThreshold.defaultValue),
  ]);
}
